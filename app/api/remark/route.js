...
export async function GET(req) {
  try {
    ...
    const remarkToId = searchParams.get("remarkToId");
    const remarkDate = searchParams.get("remarkDate");

    const remark = await Remark.find({ remarkToId, remarkDate });
    ...
    console.error("GetRemarks API Error:", error);
...
export async function POST(req) {
  try {
    ...
    if (!remark) {
      return NextResponse.json({ message: "Error occured while saving remark.", success: false }, { status: 400 });
    }

    return NextResponse.json({ data: remark, success: true }, { status: 201 });
  } catch (error) {
    console.error("SaveRemark API Error:", error);
    return NextResponse.json({ message: error.message || "Internal Server Error", success: false }, { status: 500 });
  }
}
